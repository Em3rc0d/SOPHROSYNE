...
Q03 decisions depend on behavior, not only interviews. This contract defines the minimum event model needed to reproduce E03-B, E03-C, E03-D and supportive E03-E/Q05 measurements without collecting unnecessary financial data.
...
Canonical participant protocol: `docs/validation/RESEARCH_PARTICIPANT_PROTOCOL.md`.
...
recruitment_cohort:
promotion_geo_class: PERU_ELIGIBLE | NON_PERU_EXPLORATORY | UNKNOWN
```

`promotion_geo_class` is a study eligibility classification, not precise location telemetry. Do not store street/location coordinates for this purpose.
...
context:
  ...
  price_band_id:
  component_id:
  component_exposure_rule_version:
...
Server/research-pipeline event establishing that a participant could legitimately encounter a component under the frozen progressive-disclosure/assignment rule.
...
Eligibility alone does not prove exposure.

### `component_exposed`

Emitted only when the frozen exposure rule is satisfied.

```yaml
component_id:
exposure_rule_version:
exposure_source: UI_RENDERED | TASK_ASSIGNED | FLOW_REACHED | OTHER_PRE_REGISTERED
exposure_instance_id:
```

Rules:
- mere existence of a component in the product does not count as exposure;
- exposure semantics are frozen before E05-B analysis;
- duplicate/retry exposures are deduplicated through `exposure_instance_id` or event idempotency logic;
- a participant cannot enter a component's adoption denominator without both eligibility and valid exposure when the component is not universally exposed.
...
A component-specific adoption rule defines which use events qualify for U1/U3.
...
- `promotion_geo_class` satisfies the primary/exploratory rule for the analyzed cohort;
...
## Eligible exposed repeat user for Q05

For one component:

```text
eligible_exposed_repeat_user =
  repeat-user condition true
  AND valid component_eligible exists
  AND valid component_exposed exists
  AND participant satisfies the cohort/geography rule
```

The denominator is reconstructed from events plus the frozen component eligibility/exposure contract. It is never inferred from “feature was available in that release”.
...
### DQ8 — geography/cohort integrity

Every participant used for a Peru-promotable primary metric has a valid pre-registered `promotion_geo_class`/cohort classification. `UNKNOWN` cannot silently count toward the Peru primary denominator.

### DQ9 — component exposure integrity

For Q05 component-level U1/U2/U3 metrics:
- denominator members must be reconstructable as eligible + exposed;
- a missing exposure record cannot be interpreted as “exposed but did not adopt”;
- exposure events must reference the frozen exposure-rule version;
- if exposure instrumentation is materially incomplete, the affected component result is `INCONCLUSIVE`.
...
If an instrumentation defect prevents reconstructing more than 10% of a component's eligible-exposed denominator, that component cannot receive STRONG or negative Q05 linkage classification from that run unless a pre-registered recovery rule exists.
...
- precise device/geolocation coordinates solely to establish Peru eligibility;
...
- component eligibility/exposure rule;
- component coding rubric;
- scoring rubric semantics;
- promotion geography/cohort semantics.
...
component_exposure_contract_digest_if_applicable:
promotion_geography_rule_digest:
```
...
> If we cannot reconstruct exactly why a participant counted in a numerator or denominator — including geography, eligibility and component exposure — the metric is not promotion-grade evidence.
